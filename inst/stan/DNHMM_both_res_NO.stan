...
model {
  ...
  array[T] vector[K] log_omega;
  {
    ...
    int eventChoice;
    for (n in 1:K) {
      xbRate = Xrate * betaRate[n] + offsetInt;
      ...
      for (t in 1:Nres) {
        ...
        for (event in resA[t]:(resA[t + 1] - 1)) {
          acum += -timespan[event] *
            exp(log_sum_exp(xbRate[startRate[event]:endRate[event]]));
          if (isDependent[event]) {
            acum += xbRate[choseRate[event]] +
              xbChoice[choseChoice[eventChoice]] -
              log_sum_exp(
                xbChoice[startChoice[eventChoice]:endChoice[eventChoice]]
              );
            eventChoice += 1;
          }
        }
