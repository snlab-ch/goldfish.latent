...
model {
  array[kR] vector[kR] alphas;
  for (n in 1:kR) {
    alphas[n] = rep_vector(alpha[2], kR);
    alphas[n][n] = alpha[1];
    target += dirichlet_lpdf(theta[n] | alphas[n]); // prior for trans probs
  ...
  for (n in 1:kR) {// first observation
    ...
    for (event in resA[1]:(resA[2] - 1))
      lp[n] += isDependent[event] * xb[n][choseRate[event]] -
        timespan[event] *
  ...
  for (t in 2:Nres) { // looping over observations
    for (n in 1:kR) { // looping over states
      ...
      for (event in resA[t]:(resA[t + 1] - 1))
        lp_p1[n] += isDependent[event] * xb[n][choseRate[event]] -
          timespan[event] *
          exp(log_sum_exp(xb[n][startRate[event]:endRate[event]]));
    }
